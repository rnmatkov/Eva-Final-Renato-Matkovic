...
import { taskCreateSchema, taskUpdateSchema } from "../models/validators/taskSchemas"
...
export default class TaskController {
    ...
    public readonly create = async (req: Request, res: Response) => {
        ...
        try {
            await taskCreateSchema.validateAsync(task)
        } catch (error) {
    ...
    public readonly update = async (req: Request, res: Response) => {
        ...
        try {
            await taskUpdateSchema.validateAsync(task)
        } catch (error) {
            res.status(400).json({ message: error.message })
            return
        }
